...
With telemetry generated by Sysmon, the next step was to centralize it. I configured a local instance of Splunk Enterprise to ingest the logs in real-time.

### 🚧 Challenge: Manual Configuration
...
1.  **Service Privilege Escalation:** I verified that the **Splunkd** service was running as a low-privilege account (`NT SERVICE`). I changed it to **Local System** via `services.msc` to ensure it had permissions to read the secured Sysmon logs.
